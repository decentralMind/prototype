...
contract PaymentGateway is Community {
    ...
    modifier addressValidation(address paymentAddress) {
        require(
            !_validGatewayAddress[paymentAddress],
            "Address already registered as payment gateway."
        );
        require(
            !checkIfRegistered(paymentAddress),
            "Address already registered as community"
        );
        require(
            !checkIfTrusted(paymentAddress),
            "Address already registered as trusted"
        );
        _;
    }
