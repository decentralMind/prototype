...
contract CommunityTest {
    ...
    function shouldDirectlyRemoveCommunityByOwner() public {
        ...
        Assert.equal(cr.checkIfRegistered(acc0), false, 'Account should not belabel as registered.');
        Assert.equal(cr.checkIfTrusted(acc0), false, 'Account should not be label as trusetd.');
        
        Assert.equal(cr.getTotalSupply(), 50, 'Total supply should reduced if removed account had balance in it.');
    }
